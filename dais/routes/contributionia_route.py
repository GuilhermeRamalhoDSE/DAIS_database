...
from dais.schemas.contributionia_schema import ContributionIACreateSchema, ContributionIAUpdateSchema, ContributionIASchema
...
def create_contribution(request, contribution_in: ContributionIACreateSchema, file: UploadedFile = File(...)):
    ...
    if not user_info.get('is_superuser') and str(layer.periodia.client.license_id) != str(user_info.get('license_id')):
        raise HttpError(403, "You do not have permission to add contributions to this layer.")

    contribution = ContributionIA.objects.create(**contribution_in.dict(), file=file)
    return 201, ContributionIASchema.from_orm(contribution)
